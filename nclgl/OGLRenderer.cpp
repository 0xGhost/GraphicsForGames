...
void OGLRenderer::SetShaderLight(const Light *l) {
	...
	for (size_t i = 0; i < 4; i++)
	{
		...
		positions[i+1] = l[i].GetPosition().y;
		positions[i+2] = l[i].GetPosition().z;
		colour[i] = l[i].GetColour().x;
		colour[i+1] = l[i].GetColour().y;
		colour[i+2] = l[i].GetColour().z;
		colour[i+3] = l[i].GetColour().x;
		specularColour[i] = l[i].GetSpecularColour().x;
		specularColour[i+1] = l[i].GetSpecularColour().y;
